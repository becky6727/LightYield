...
#read data
PPOArray, EArray, QArray, ErrQArray = numpy.loadtxt(FileIN, skiprows = 0, comments = '#', unpack = True)
ErrPPOArray = [0.0 for j in xrange(len(PPOArray))]
...
for i in xrange(len(gArray)):
    ...
    XArray = numpy.array(PPOArray)[numpy.where(EArray == SourceType[i])]
    ErrXArray = numpy.array(ErrPPOArray)[numpy.where(EArray == SourceType[i])]
    ...
    #gArray[i].SetTitle('PPO Concentration Dependence of Charge at Compton edge')
    gArray[i].SetTitle('')
    gArray[i].GetXaxis().SetTitle('PPO Concentration [g/L]')
