...
for i in range(len(tr)):
    ...
    tr[i].Draw('ADC>>hADC_%d_Cut' %(i), CutPGA + CutDWT, 'Q0')
...
for i in range(len(hADC)):
    ...
    #normalize
    #Norm = hADC[i].GetEntries()
    Norm = Duaration[i]

    print '%s:' %(FileIN[i])
    print 'Duaration time = %.3f sec' %(Norm)
    print 'Event rate = %.3f Hz' %(hADC[i].GetEntries()/Norm)
    print 'Event rate = %.3f Hz after noise reduction' %(hADC_Cut[i].GetEntries()/Norm)
...
#figure
if(isFigOut):
    FileName = re.findall('./Output/(.*)_(.*).root', FileIN[0])
